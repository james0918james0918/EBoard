...
def yktasyDS_crawler():
    ...
    conn = conn_to_db()
    cur = conn.cursor()
    url = 'http://www.im.ntu.edu.tw/~tsay/dokuwiki/doku.php?id=courses:ds2016:main' #DS

    website_info_insert(cur, "010", "DS", "課程", url)
    ...
    for each in announcementTexts:
      #  print(each)
        cur.execute(sql,(each[0],each[1],each[2],each[3],'announcements','010'))
